...
agg AS (
  SELECT
    ...
    /* Fabricante CNPJ (CMED - manufacturer_code contém CNPJ) */
    MAX(b.manufacturer_code)
      FILTER (WHERE b.source_id = s.cmed_source_id)
      AS fabricante_cnpj,
...
-- Update comment
COMMENT ON VIEW public.vw_ref_item_unified IS 'View unificada de itens de referência (CMED, Brasíndice, Simpro) para associação com produtos - inclui quantidade e CNPJ do fabricante';
