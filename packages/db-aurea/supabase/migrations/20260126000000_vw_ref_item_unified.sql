-- Migration: Create unified reference items view
-- This view consolidates medication data from CMED, Brasíndice, and Simpro sources
-- Used to fetch medication data for product association
...
agg AS (
  SELECT
    ...
    MAX(NULLIF(btrim(b.manufacturer_name), ''))
      FILTER (WHERE b.source_id = s.brasindice_source_id) AS fabricante,
...
COMMENT ON VIEW public.vw_ref_item_unified IS 'View unificada de itens de referência (CMED, Brasíndice, Simpro) para associação com produtos';
