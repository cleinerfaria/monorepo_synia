 ...
 *
 * Uso:
 *   node scripts/setup-dev.cjs [aurea|white-label|all]
 */
...
const PROJECTS = ['aurea', 'white-label']
const DEFAULT_PROJECT = 'aurea'
    ...
    proc.on('close', (code) => {
      if (code === 0) {
        ...
        reject(new Error(
          ...
          '  4. Tente novamente: npm run setup:dev:aurea'
        ))
      }
    })
...
function displaySetupInfo() {
  ...
  log(`     cp apps/aurea/.env.example apps/aurea/.env.local`, 'dim')
  log(`     (ou white-label/.env.local)`, 'dim')

  log(`\n  2. Inicie o desenvolvimento:`, 'dim')
  log(`     npm run dev:aurea`, 'dim')
  log(`     ou`, 'dim')
  ...
  log(`  Admin:   e2e.admin@aurea.local / AureaE2E!123`, 'dim')
  log(`  Manager: e2e.manager@aurea.local / AureaE2E!123`, 'dim')
  log(`  User:    e2e.user@aurea.local / AureaE2E!123`, 'dim')
