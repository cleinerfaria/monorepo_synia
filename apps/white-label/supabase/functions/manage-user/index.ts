...
serve(async (req) => {
  ...
  try {
    ...
    const { data: requestingAppUser, error: _appUserError } = await supabaseAdmin
      .from('app_user')
      .select(
        `
        id,
    ...
    if (requestingAppUser) {
      isAdmin =
        requestingAppUser.access_profile?.is_admin ||
        requestingAppUser.access_profile?.code === 'admin';
    } else {
      ...
      case 'create': {
        ...
        if (!isAdmin) {
          ...
          const { data: hasPermission } = await supabaseAdmin.rpc('has_permission', {
            p_user_auth_id: requestingUser.id,
            p_module_code: 'admin',
