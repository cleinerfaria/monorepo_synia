  ...
  const validate = () => {
    ...
    if (!isEditing) {
      ...
      if (!formData.company_id) {
        newErrors.company_id = 'Empresa Ã© obrigatÃ³ria';
      }
    }
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          />
          {isEditing && (
            <p className="mt-1 text-xs text-gray-500">O e-mail nÃ£o pode ser alterado</p>
          )}
        </div>
        ...
        {/* Reset Password (apenas para ediÃ§Ã£o) */}
        {isEditing && (
          ...
          >
            ...
            {activeProfiles.map((profile) => (
              <option key={profile.id} value={profile.id}>
                {profile.name} {profile.is_system && '(Sistema)'} {profile.is_admin && 'â­'}
              </option>
            ))}
          </select>
          {errors.access_profile_id && (
            <p className="mt-1 text-sm text-red-500">{errors.access_profile_id}</p>
          )}
