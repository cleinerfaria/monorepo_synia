...
export default function LinkUserModal({ isOpen, onClose, companies }: LinkUserModalProps) {
  ...
  useEffect(() => {
    ...
    if (formData.access_profile_id) return;

    const defaultProfile =
      accessProfiles.find((p) => p.code === 'admin') ||
      accessProfiles.find((p) => p.active) ||
      accessProfiles[0];

    if (defaultProfile?.id) {
      setFormData((prev) => ({ ...prev, access_profile_id: defaultProfile.id }));
    }
  }, [accessProfiles, formData.company_id, formData.access_profile_id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAppUser({
        ...appUser,
        theme: (appUser as any).theme || 'system',
      } as any);
        ...
        <div>
          <Select
            ...
            options={[
              ...
              ...accessProfiles.map((profile) => ({ value: profile.id, label: profile.name })),
            ]}
            value={formData.access_profile_id}
            onChange={(value: string) =>
              setFormData({ ...formData, access_profile_id: value || '' })
            }
