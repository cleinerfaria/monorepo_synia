...
export async function fetchRefItemsBatchByEans(
  ...
  if (priceError) {
    ...
    console.warn(`Found ${priceData.length}/${cleanEans.length} price entries in materialized view`)
...
export function useLinkProductToRefItem() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY, 'product-links', variables.productId] })
