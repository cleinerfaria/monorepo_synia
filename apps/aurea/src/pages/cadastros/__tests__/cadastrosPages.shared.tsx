...
export type NavigationPageCase = {
  name: string;
  heading: string;
  Component: React.ComponentType;
  createButton: RegExp;
...
export const renderPage = (Component: React.ComponentType) => {
  render(<Component />);
};

export const submitModal = async (dialogTitle: string) => {
...
export const modalPages: ModalPageCase[] = [
  {
    name: 'clientes',
    heading: 'Clientes',
...
export const navigationPages: NavigationPageCase[] = [
  {
    name: 'pacientes',
    heading: 'Pacientes',
...
export const getModalPageCase = (name: ModalPageCase['name']) => {
  const page = modalPages.find((item) => item.name === name);
  if (!page) throw new Error(`Modal page case not found: ${name}`);
  return page;
};

export const getNavigationPageCase = (name: NavigationPageCase['name']) => {
  const page = navigationPages.find((item) => item.name === name);
  if (!page) throw new Error(`Navigation page case not found: ${name}`);
  return page;
};

export const defineModalPageSuite = (name: ModalPageCase['name']) => {
  const page = getModalPageCase(name);
...
export const defineNavigationPageSuite = (name: NavigationPageCase['name']) => {
  const page = getNavigationPageCase(name);
  ...
  describe(page.name, () => {
    ...
    it('aciona criação corretamente', async () => {
      ...
      expect(getCadastrosPageMocks().navigate).toHaveBeenCalledWith(page.createPath);
    });
    ...
    it('aciona edição corretamente', async () => {
      ...
      expect(getCadastrosPageMocks().navigate).toHaveBeenCalledWith(page.editPath);
    });
  });
};

export const getCadastrosPageMocks = () => mocks;
