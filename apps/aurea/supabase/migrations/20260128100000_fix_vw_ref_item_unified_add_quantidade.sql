-- Migration: Fix vw_ref_item_unified to include quantity field properly
-- The quantity field was missing from the actual database view
...
agg AS (
  SELECT
    ...
    /* Quantidade: prioriza Brasíndice, fallback SIMPRO, fallback CMED */
    COALESCE(
      MAX(b.quantity)
        FILTER (WHERE b.source_id = s.brasindice_source_id),
      MAX(b.quantity)
...
FROM agg
WHERE
  nome IS NOT NULL  -- Only require name, allow null manufacturer
ORDER BY
  nome;

-- Add comment to the view
COMMENT ON VIEW public.vw_ref_item_unified IS 'View unificada de itens de referência (CMED, Brasíndice, Simpro) para associação com produtos - inclui quantidade corrigida';